...
def home():
    ...
    return render_template('home.html', title= "The Master Blog", 
...
def AddBlog():
    ...
    if request.method == 'POST':
        ...
        if new_title == "":
            error["title_blank"] = "Give your blog a title"
        if new_body == "":
            error["body_blank"] = "Enter text for the blog"
...
def register():
    ...
    if request.method == 'POST':
        ...
        if username == "":
            error["name_error"] = "Username cannot be empty"
        if password == "":
            error["pass_error"] = "Password must contain numbers/letters"
        elif len(password) < 2:
        ...
        if existing_user:
            error["name_error"] = "That username is already in use"
